...
function extractClosedDays(text) {
  // 1. 【定休日】月 などのパターン (ホットペッパー)
  const hpMatch = text.match(/【定休日】\s*([^\s【]+)/);
  if (hpMatch) {
    return hpMatch[1].trim();
  }
  ...
  // 3. 不定休や年中無休の直接チェック
  if (text.includes('不定休')) {
    return '不定休';
  }
  if (text.includes('年中無休')) {
    return '年中無休';
  }
  ...
  return ''; // 見つからない場合は空欄にする (呼び出し元でデフォルト '無休' に処理可能)
}
